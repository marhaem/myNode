...
(function(Buffer) {
  ...
  internals.Response.prototype._setSource = function(source, variety) {
    ...
    if (source === null || source === undefined || source === '') {
      ...
      this.variety = 'stream';
    } else if (typeof source === 'object' && typeof source.then === 'function') {
  ...
  internals.Response.prototype.etag = function(tag, options) {
    ...
    this.settings.varyEtag = options.vary !== false && !options.weak;
    return this;
  };
  internals.Response.unmodified = function(request, options) {
    ...
    if (options.etag && request.headers['if-none-match']) {
    ...
    if (ifModifiedSinceHeader && options.modified) {
      const ifModifiedSince = internals.parseDate(ifModifiedSinceHeader);
      const lastModified = internals.parseDate(options.modified);
      if (ifModifiedSince && lastModified && ifModifiedSince >= lastModified) {
        return true;
      }
    }
    return false;
  ...
  internals.Response.prototype.state = function(name, value, options) {
      ...
      if (this.source.headers) {
        ...
        if (headerKeys.length) {
          ...
          for (let i = 0; i < headerKeys.length; ++i) {
            ...
            this.header(key.toLowerCase(), Hoek.clone(this.source.headers[key]));
          }
  ...
  internals.Response.prototype._streamify = function(source, next) {
    ...
    let payload = source;
    if (this.variety === 'plain' && source !== null && typeof source !== 'string') {
  ...
  internals.Payload.prototype._read = function() {
    if (this._prefix) {
