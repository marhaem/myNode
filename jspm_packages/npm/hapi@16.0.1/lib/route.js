...
exports = module.exports = internals.Route = function(route, connection, plugin, options) {
  ...
  if (realm.modifiers.route.vhost || realm.modifiers.route.prefix) {
    route = Hoek.cloneWithShallow(route, ['config']);
  ...
  Hoek.assert(!!route.handler ^ !!(config && config.handler), 'Handler must only appear once:', routeDisplay);
  ...
  this.public = {
    ...
    auth: {access: (request) => Auth.access(request, this.public)}
  };
  ...
  if (this.method === 'get') {
    ...
    this.settings.payload.decoders = this.connection._compression._decoders;
  }
  ...
  this._extensions = {onPreResponse: this._combineExtensions('onPreResponse')};
  if (this._special) {
...
internals.Route.prototype.rebuild = function(event) {
  ...
  const authenticate = (this.settings.auth !== false);
  if (authenticate) {
  ...
  cycle.push(Handler.execute);
  if (this._extensions.onPostHandler.nodes) {
    cycle.push(this._extensions.onPostHandler);
  }
...
internals.state = function(request, next) {
  ...
  request.connection.states.parse(cookies, (err, state, failed) => {
    ...
    if (!err || request.route.settings.state.failAction === 'ignore') {
      return next();
    }
    request._log(['state', 'error'], {
      header: cookies,
      errors: err.data
    });
...
internals.payload = function(request, next) {
  ...
  Subtext.parse(request.raw.req, request._tap(), request.route.settings.payload, (err, parsed) => {
    if (!err || !request._isPayloadPending) {
