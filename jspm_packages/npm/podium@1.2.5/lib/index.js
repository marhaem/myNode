...
(function(process) {
  ...
  const internals = {schema: {base: Joi.object({
  ...
  internals.schema.event = internals.schema.base.keys({shared: Joi.boolean()});
    ...
    [].concat(podiums).forEach((podium) => {
      ...
      Object.keys(podium._eventListeners).forEach((name) => {
        if (!this._eventListeners[name]) {
          this._eventListeners[name] = {
            handlers: null,
            flags: podium._eventListeners[name].flags
          };
        }
      });
    });
  ...
  internals.emit = function(emitter, notification) {
    ...
    if (emitter._eventsProcessing || !emitter._notificationsQueue.length) {
      return;
    }
    ...
    const relay = () => {
      ...
      const each = (podium, next) => podium._emit(item.criteria, data, generated, next);
    ...
    const each = (handler, next) => {
      ...
      const invoke = (func) => {
        if (handler.channels && (!item.criteria.channel || handler.channels.indexOf(item.criteria.channel) === -1)) {
          return;
        }
        if (handler.filter) {
        ...
        if (internals.flag('tags', handler, event) && item.criteria.tags) {
    ...
    return Items.parallel(handlers.slice(), each, relay);
  };
