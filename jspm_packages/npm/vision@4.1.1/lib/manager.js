...
(function(process) {
  ...
  internals.schema.view = internals.schema.viewBase.keys({module: Joi.object({compile: Joi.func().required()}).options({allowUnknown: true}).required()});
  exports = module.exports = internals.Manager = function(options) {
    ...
    extensions.forEach((extension) => {
      ...
      const engine = {};
      if (config.compile && typeof config.compile === 'function') {
  ...
  internals.Manager.prototype._loadPartials = function(engine) {
    if (!engine.config.partialsPath || !engine.module.registerPartial || typeof engine.module.registerPartial !== 'function') {
      return;
    }
    const load = function() {
    ...
    const traverse = function(path) {
      ...
      Fs.readdirSync(path).forEach((file) => {
        ...
        if (Path.basename(file)[0] !== '.' && Path.extname(file) === engine.suffix) {
          files.push(file);
        }
      });
      return files;
  ...
  internals.Manager.prototype._loadHelpers = function(engine) {
    if (!engine.config.helpersPath || !engine.module.registerHelper || typeof engine.module.registerHelper !== 'function') {
      return;
    }
    ...
    helpersPaths.forEach((helpersPath) => {
      ...
      Fs.readdirSync(path).forEach((file) => {
        file = Path.join(path, file);
        const stat = Fs.statSync(file);
        if (!stat.isDirectory() && Path.basename(file)[0] !== '.') {
          try {
  ...
  internals.Manager.prototype._prepareTemplates = function(template, engine, options, callback) {
    const compiled = {settings: Hoek.applyToDefaults(engine.config, options)};
  ...
  internals.Manager.prototype._path = function(template, settings, isLayout, next) {
    ...
    if (!settings.allowAbsolutePaths && isAbsolutePath) {
      return next(Boom.badImplementation('Absolute paths are not allowed in views'));
    }
    if (!settings.allowInsecureAccess && isInsecurePath) {
  ...
  internals.Manager.prototype._render = function(compiled, context, request, callback) {
    ...
    if (compiled.layout && context.hasOwnProperty(compiled.settings.layoutKeyword)) {
      return callback(Boom.badImplementation('settings.layoutKeyword conflict', {
        context,
        keyword: compiled.settings.layoutKeyword
      }));
    }
  ...
  internals.Manager.prototype._compile = function(template, engine, settings, callback) {
    if (engine.cache && engine.cache[template]) {
      return callback(null, engine.cache[template]);
    }
    settings.compileOptions.filename = template;
