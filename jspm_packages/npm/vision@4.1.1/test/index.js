...
describe('handler()', () => {
  ...
  it('handles custom context', (done) => {
    ...
    server.route({
      ...
      path: '/',
      handler: {view: {
          template: 'valid/index',
          context: {message: 'heyloo'}
        }}
    });
    server.inject('/', (res) => {
  ...
  it('handles custom options', (done) => {
    ...
    server.route({
      ...
      path: '/',
      handler: {view: {
          template: 'valid/options',
          options: {layout: 'elsewhere'}
        }}
    });
    server.inject('/', (res) => {
      expect(res.result).to.contain('+hello');
  ...
  it('handles a global context object', (done) => {
    ...
    server.views({
      engines: {html: require('handlebars')},
      path: __dirname + '/templates',
      context: {message: 'default message'}
    });
    server.route({
      method: 'GET',
      path: '/',
      handler: {view: {template: 'valid/testContext'}}
    });
    server.inject('/', (res) => {
  ...
  it('passes the request to a global context function', (done) => {
    ...
    server.views({
      ...
      path: __dirname + '/templates',
      context: (request) => {
        return {message: request ? request.route.path : 'default'};
      }
    });
    server.route({
      method: 'GET',
...
describe('render()', () => {
  ...
  it('renders view (root with options)', (done) => {
    ...
    server.render('test', {
      title: 'test',
      message: 'Hapi'
    }, {path: Path.join(__dirname, '/templates/valid')}, (err, rendered, config) => {
      expect(err).not.to.exist();
      expect(rendered).to.exist();
      expect(rendered).to.contain('Hapi');
      done();
  ...
  it('returns a promise when no options or callback given (server)', () => {
    ...
    return server.render('test', {message: 'Hello!'}).then((content) => expect(content).to.contain('<h1>Hello!</h1>'));
  });
