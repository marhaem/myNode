...
describe('Manager', () => {
  ...
  it('errors absolute path given and allowAbsolutePath is false (by default)', (done) => {
    ...
    server.route({
      ...
      path: '/',
      handler: {view: {
          template: __dirname + '/templates/valid/test',
          context: {message: 'Hello, World!'}
        }}
    });
    server.inject('/', (res) => {
  ...
  it('errors if engine.compile throws', (done) => {
    ...
    server.route({
      ...
      path: '/',
      handler: {view: {
          template: 'badmustache',
          context: {message: 'Hello, World!'},
          options: {path: __dirname + '/templates/valid/invalid'}
        }}
    });
    server.inject('/', (res) => {
      expect(res.result).to.exist();
  ...
  it('should not fail if rendered template returns undefined', (done) => {
    ...
    server.register(Vision, Hoek.ignore);
    server.views({engines: {html: {
          module: {compile: function(template, compileOptions) {
    ...
    server.route({
      method: 'GET',
      path: '/',
      handler: {view: {template: 'test.html'}}
    });
    server.inject('/', (res) => {
      expect(res.statusCode).to.equal(200);
      done();
    ...
    it('renders handlebars template', (done) => {
      ...
      server.views({
        ...
        engines: {
          ...
          'pug': require('pug'),
          'hbar': {module: {compile: function(engine) {
  ...
  describe('render()', () => {
    it('renders with async compile', (done) => {
      const views = new Manager({
        ...
        engines: {html: {
            compileMode: 'async',
            module: {compile: function(string, options, callback) {
      ...
      views.render('valid/test', {
        title: 'test',
        message: 'Hapi'
      }, null, (err, rendered, config) => {
    ...
    it('errors on sync compile that throws', (done) => {
      const views = new Manager({
        ...
        engines: {html: {
            compileMode: 'sync',
            module: {compile: function(string, options) {
    ...
    it('allows valid (with layouts)', (done) => {
      ...
      testViewWithLayouts.render('valid/test', {
        title: 'test',
        message: 'Hapi'
      }, null, (err, rendered, config) => {
        expect(err).not.to.exist();
    ...
    it('allows valid pug layouts', (done) => {
      ...
      testViewWithJadeLayouts.render('index', {
        title: 'test',
        message: 'Hapi'
      }, null, (err, rendered, config) => {
        expect(err).not.to.exist();
    ...
    it('should work and not throw without pug layouts', (done) => {
      ...
      testViewWithoutJadeLayouts.render('test', {
        title: 'test',
        message: 'Hapi Message'
      }, null, (err, rendered, config) => {
        expect(err).not.to.exist();
    ...
    it('allows relativeTo, template name, and no path', (done) => {
      ...
      views.render('test', {
        title: 'test',
        message: 'Hapi'
      }, {relativeTo: __dirname + '/templates/valid'}, (err, rendered, config) => {
        expect(err).not.to.exist();
        expect(rendered).to.exist();
        expect(rendered).to.contain('Hapi');
        done();
    ...
    it('errors when referencing non existant partial (with layouts)', (done) => {
      ...
      testViewWithLayouts.render('invalid/test', {
        title: 'test',
        message: 'Hapi'
      }, null, (err, rendered, config) => {
        expect(err).to.exist();
    ...
    it('errors on compile error (invalid template code)', (done) => {
      ...
      testView.render('invalid/badmustache', {
        title: 'test',
        message: 'Hapi'
      }, null, (err, rendered, config) => {
    ...
    it('does not register helpers on engines that don\'t have helper support', (done) => {
      const tempView = new Manager({
        engines: {html: {compile: function(template) {
    ...
    it('reuses cached compilation', (done) => {
      ...
      views.render('valid/test', {
        title: 'test',
        message: 'Hapi'
      }, null, (err, original, originalConfig) => {
        expect(err).not.to.exist();
    ...
    it('returns a promise that is resolved with the content if no callback is provided', () => {
      ...
      return views.render('test', {message: 'Hello!'}, null).then((content) => {
